...
struct OpenHABRootView: View {
    ...
    private func persistIfChanged<V: Equatable & Sendable>(_ keyPath: WritableKeyPath<HomePreferences, V>, _ value: V) {
        guard cachedHomePrefs?[keyPath: keyPath] != value else { return }
        Task {
            await Preferences.shared.modifyActiveHome { @Sendable prefs in prefs[keyPath: keyPath] = value }
        }
    }
